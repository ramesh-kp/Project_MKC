import { graphql } from "@keystone-6/core";
import { virtual } from "@keystone-6/core/fields";
import { KeystoneContext } from "@keystone-6/core/types";

const currentResolverGen =
  (deviceConditionId: string) =>
  async (
    item: any,
    all: { from: Date; to: Date },
    context: KeystoneContext
  ) => {
    const device: { [key: string]: string } = {};
    device[deviceConditionId] = item.id;
    const result = await context.prisma.deviceRead.findFirst({
      select: {
        value: true,
      },
      orderBy: {
        createdAt: "desc",
      },
      where: {
        createdAt: {
          gte: new Date(all.from),
          lte: new Date(all.to),
        },
        device,
      },
    });
    return result?.value || (0 as number);
  };
export const currentField = (deviceConditionId: string) =>
  virtual({
    field: graphql.field({
      type: graphql.Float,
      args: {
        from: graphql.arg({
          type: graphql.nonNull(graphql.DateTime),
          defaultValue: new Date(
            new Date().getTime() - 1000 * 60 * 60 * 24 * 7
          ),
        }),
        to: graphql.arg({
          type: graphql.nonNull(graphql.DateTime),
          defaultValue: new Date(),
        }),
      },
      resolve: currentResolverGen(deviceConditionId),
    }),
  });
