...
from general_configurations import ATTRIBUTEERROR, COD_ARRAY_POS_1, COD_ARRAY_POS_2
from general_configurations import CONFIGURATIONS_FILE, READ_MODE, POSITION_2, POSITION_1
from general_configurations import FLAG_STATUS_0 , COD, INDEXERROR, VALUEERROR, Error_number
from general_configurations import INFO_NOT_FOUND_IN_FILE, INDENT_LEVEL, INCREMENT_1
from general_configurations import ADDED_DEVICE_SUCCESSFULLY, COD_SENSOR_NOT_FOUND
...
from general_configurations import ERROR_IN_ENABLE_READING, ERROR_IN_SET_COD_CONFIGURATIONS
...
class CODSensor(object):
    """
    This class contain functions which initilize configurations of COD and its array position
    for getting the COD data from the COD reading.It contain functions which reads the data from
    COD Sensor.
    """
    ...
    def __init__(self,deviceid = None):
        ...
        self.cod_start_flag = FLAG_STATUS_0
        self.cod_configurations = None
        self.cod_data_position = [COD_ARRAY_POS_1 , COD_ARRAY_POS_2 ]
        self.cod_configurations = self.get_cod_configurations()
    def get_cod_configurations(self):
        """
        Description: Returns the cod configurations from Sensor_Configuration_File.
    ...
    def validate_content(self, sensor_reading, position):
        ...
        try:
            ...
            if self.validate_message(sensor_reading_check):
                ...
                sensor_result.update({SENSORGETDEVICEID.format(COD, position):
                    self.cod_configurations[position][ARRAY_POSITION_0 ],
                    ERRORMESSAGESENSOR.format(COD, position): result})
            else:
                sensor_result.update({SENSORGETDEVICEID.format(COD, position):
                    self.cod_configurations[position][ARRAY_POSITION_0 ],
                    SENSORGETDATA.format(COD, position): Conversion().parse_rawdata(
                        sensor_reading.registers,self.cod_data_position[POSITION_1],
                        self.cod_data_position[POSITION_2])})
    ...
    def get_cod_reading(self, cod_start_flag):
        ...
        Input Parameters: cod_start_flag which is either True or False
        Output Type: dictionary
        """
        try:
            cod_data = {}
            for position, _ in enumerate(self.cod_configurations):
                while True:
                    if (cod_start_flag == FLAG_STATUS_1) and (self.deviceid ==
                                self.cod_configurations[position][ARRAY_POSITION_0 ]):
                        cod_reading = (DeviceCommunication.gateway_connect().read_holding_registers(
                            count=self.cod_configurations[position][ARRAY_POSITION_2],
                            address=self.cod_configurations[position][ARRAY_POSITION_1 ],
                            unit=self.cod_configurations[position][ARRAY_POSITION_0]))
                        cod_data.update(self.validate_content(cod_reading, position))
                        Datalogs.get_instance().logging_error(RECEIVED_READINGS_FROM_SENSOR,
                                    SENSORSERROR.format(COD, self.deviceid), Log_levels[INFO],
                                    self.deviceid)
            ...
            Datalogs.get_instance().logging_error((ex, Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(COD, self.deviceid),
                                                  Log_levels[ERROR], self.deviceid)
            return {ERRORMESSAGESENSOR.format(COD, self.deviceid):ex}
        except Exception as ex:
            if type(ex).__name__ == CONNECTIONEXCEPTION:
                result = {ERRORMESSAGESENSOR.format(COD, self.deviceid):MODBUSCONNECTIONERROR}
            else:
                result = {ERRORMESSAGESENSOR.format(COD, self.deviceid):ex}
    ...
    def enable_reading(self):
        ...
        """
        try:
            if self.cod_configurations is not None:
                self.cod_start_flag = FLAG_STATUS_1
                ...
                Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(Matching(
                        ...
                        SENSORSERROR.format(COD, self.deviceid), Log_levels[ERROR], self.deviceid)
            return {COD:result}
        except IndexError as ex:
            Datalogs.get_instance().logging_error((ex,Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(COD, self.deviceid),
    ...
    def diasble_reading(self):
        ...
        try:
            ...
            Datalogs.get_instance().logging_error(READING_DISABLED,SENSORSERROR.format(
                COD, self.deviceid), Log_levels[INFO], self.deviceid)
            ...
            while count < 3:
                with open(SENSOR_ADDRESS_FILE, READ_MODE, encoding="utf-8") as file:
                    ...
                    sensor_address = data[SENSORGETADDRESS[INITIAL_INDEX :FINAL_INDEX ]
                                        ][SENSORGETSTARTADDRESS[:FINAL_INDEX ].format(COD)]
                    sensor_id = data[SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                    ][SENSORGETDEFAULTID[:FINAL_INDEX ].format(COD)]
                    ...
                    Datalogs.get_instance().logging_error(ADDED_DEVICE_SUCCESSFULLY,
                                    SENSORSERROR.format(COD), Log_levels[INFO], self.deviceid)
                    result = ADDED_DEVICE_SUCCESSFULLY
            if count >= 3:
    ...
    def set_cod_configurations(self, data):
        ...
        try:
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                file_data = json.load(file)
                if list((data).keys())[INDEX_POSITION_0 ] == COD:
                    ...
                    count = COUNT_0
                    for position in range(len(file_data[SENSORCONFIGURATIONS.format(COD)])):
                        if file_data[SENSORCONFIGURATIONS.format(COD)][position][SENSORGETDEVICEID[
                    ...
                    Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(
                        INVALID_INFO, ERROR_IN_SET_COD_CONFIGURATIONS)),SENSORSERROR.format(
                            COD, self.deviceid), Log_levels[ERROR],
                        self.deviceid)
    ...
    def remove_cod_configurations(self):
        ...
        try:
            ...
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                ...
                for index, _ in enumerate(len(cod_data)):
                    if cod_data[index][SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                       ] == self.deviceid:
                ...
                if check_con == COUNT_0 :
                    Datalogs.get_instance().logging_error(COD_SENSOR_NOT_FOUND.format(
                        self.deviceid), SENSORSERROR.format(COD, self.deviceid),
                                                        Log_levels[ERROR], self.deviceid)
                    result = COD_SENSOR_NOT_FOUND.format(self.deviceid)
