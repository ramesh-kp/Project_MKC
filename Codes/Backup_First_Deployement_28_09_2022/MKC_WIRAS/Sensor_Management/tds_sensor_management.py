...
from general_configurations import FLAG_STATUS_0 , TDS, INDEXERROR, VALUEERROR, Error_number
from general_configurations import INFO_NOT_FOUND_IN_FILE, INDENT_LEVEL, INCREMENT_1
from general_configurations import ADDED_DEVICE_SUCCESSFULLY, TDS_SENSOR_NOT_FOUND
...
from general_configurations import (
    ...
    ERROR_IN_SET_TDS_CONFIGURATIONS,
)
from general_configurations import (
    INFO,
...
class TDSSensor(object):
    """
    This class contain functions which initilize configurations of TDS and its array position
    for getting the TDS data from the TDS reading.It contain functions which reads the data from
    TDS Sensor.
    """
    ...
    def __init__(self,deviceid = None):
        ...
        self.tds_start_flag = FLAG_STATUS_0
        self.tds_configurations = None
        self.tds_data_position = [TDS_ARRAY_POS_1 , TDS_ARRAY_POS_2 ]
        self.tds_configurations = self.get_tds_configurations()
    def get_tds_configurations(self):
        """
        Description: Returns the tds configurations from Sensor_Configuration_File.
    ...
    def validate_content(self, sensor_reading, position):
        ...
        try:
            ...
            if self.validate_message(sensor_reading_check):
                ...
                sensor_result.update({SENSORGETDEVICEID.format(TDS, position):
                    self.tds_configurations[position][ARRAY_POSITION_0 ],
                    ERRORMESSAGESENSOR.format(TDS, position): result})
            else:
                sensor_result.update({SENSORGETDEVICEID.format(TDS, position):
                    self.tds_configurations[position][ARRAY_POSITION_0 ],
                    SENSORGETDATA.format(TDS, position): Conversion().parse_rawdata(
                        sensor_reading.registers,self.tds_data_position[POSITION_1],
                        self.tds_data_position[POSITION_2])})
    ...
    def get_tds_reading(self, tds_start_flag):
        """
        Description: Returns the final output from the sensor readings
        Input Parameters: tds_start_flag which is either True or False
        Output Type: dictionary
        """
        try:
            tds_data = {}
            for position, _ in enumerate(self.tds_configurations):
                while True:
                    if (tds_start_flag == FLAG_STATUS_1) and (self.deviceid ==
                                self.tds_configurations[position][ARRAY_POSITION_0 ]):
                        tds_reading = (DeviceCommunication.gateway_connect().read_holding_registers(
                            count = self.tds_configurations[position][ARRAY_POSITION_2],
                            address=self.tds_configurations[position][ARRAY_POSITION_1 ],
                            unit=self.tds_configurations[position][ARRAY_POSITION_0]))
                        tds_data.update(self.validate_content(tds_reading, position))
                        Datalogs.get_instance().logging_error(RECEIVED_READINGS_FROM_SENSOR,
                                    SENSORSERROR.format(TDS, self.deviceid), Log_levels[INFO],
                                    self.deviceid)
            ...
            Datalogs.get_instance().logging_error((ex, Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(TDS, self.deviceid),
                                                  Log_levels[ERROR], self.deviceid)
            return {ERRORMESSAGESENSOR.format(TDS, self.deviceid):ex}
        except Exception as ex:
            if type(ex).__name__ == CONNECTIONEXCEPTION:
                result = {ERRORMESSAGESENSOR.format(TDS, self.deviceid):MODBUSCONNECTIONERROR}
            else:
                result = {ERRORMESSAGESENSOR.format(TDS, self.deviceid):ex}
    ...
    def enable_reading(self):
        ...
        """
        try:
            if self.tds_configurations is not None:
                self.tds_start_flag = FLAG_STATUS_1
                result = self.get_tds_reading(self.tds_start_flag)
            else:
                ...
                Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(Matching(
                        ...
                        SENSORSERROR.format(TDS, self.deviceid), Log_levels[ERROR], self.deviceid)
            return {TDS:result}
        except IndexError as ex:
            Datalogs.get_instance().logging_error((ex,Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(TDS, self.deviceid),
    ...
    def diasble_reading(self):
        ...
        try:
            self.tds_start_flag = FLAG_STATUS_0
            self.get_tds_reading(self.tds_start_flag)
            Datalogs.get_instance().logging_error(READING_DISABLED,SENSORSERROR.format(
                TDS, self.deviceid), Log_levels[INFO], self.deviceid)
            ...
            while count < 3:
                with open(SENSOR_ADDRESS_FILE, READ_MODE, encoding="utf-8") as file:
                    ...
                    sensor_address = data[SENSORGETADDRESS[INITIAL_INDEX :FINAL_INDEX ]
                                        ][SENSORGETSTARTADDRESS[:FINAL_INDEX ].format(TDS)]
                    sensor_id = data[SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                    ][SENSORGETDEFAULTID[:FINAL_INDEX ].format(TDS)]
                    ...
                    Datalogs.get_instance().logging_error(ADDED_DEVICE_SUCCESSFULLY,
                                    SENSORSERROR.format(TDS), Log_levels[INFO], self.deviceid)
                    result = ADDED_DEVICE_SUCCESSFULLY
            if count >= 3:
    ...
    def set_tds_configurations(self, data):
        ...
        try:
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                file_data = json.load(file)
                if list((data).keys())[INDEX_POSITION_0 ] == TDS:
                    ...
                    count = COUNT_0
                    for position in range(len(file_data[SENSORCONFIGURATIONS.format(TDS)])):
                        if file_data[SENSORCONFIGURATIONS.format(TDS)][position][SENSORGETDEVICEID[
                    ...
                    Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(
                        INVALID_INFO, ERROR_IN_SET_TDS_CONFIGURATIONS)),SENSORSERROR.format(
                            TDS, self.deviceid), Log_levels[ERROR],
                        self.deviceid)
    ...
    def remove_tds_configurations(self):
        """
        ...
        try:
            ...
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                ...
                for index, _ in enumerate(len(tds_data)):
                    if tds_data[index][SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                       ] == self.deviceid:
                        check_con = check_con + INCREMENT_1
                        self.tds_reinitiliaze()
                        del tds_data[index]
                        result = CONFIGURATIONS_REMOVED_SUCCESSFULLY
                if check_con == COUNT_0 :
                    Datalogs.get_instance().logging_error(TDS_SENSOR_NOT_FOUND.format(
                        self.deviceid), SENSORSERROR.format(TDS, self.deviceid),
                                                        Log_levels[ERROR], self.deviceid)
                    result = TDS_SENSOR_NOT_FOUND.format(self.deviceid)
