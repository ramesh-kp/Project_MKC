...
from general_configurations import FLAG_STATUS_0 , BOD, INDEXERROR, VALUEERROR, Error_number
            ...
            with open(CONFIGURATIONS_FILE, READ_MODE, encoding="utf-8") as file:
                ...
                if file_data and SENSORCONFIGURATIONS.format(BOD) in file_data:
                    ...
                    Datalogs().logging_error(Matching().error_mapping(INFO_NOT_FOUND_IN_FILE),
                        SENSORSERROR.format(BOD,self.deviceid), Log_levels[DEBUG], self.deviceid)
                return result
        except FileNotFoundError as ex:
            Datalogs.get_instance().logging_error((ex, Error_number[FILENOTFOUND]),
                                                  SENSORSERROR.format(BOD,self.deviceid),
            ...
            if self.validate_message(sensor_reading_check):
                ...
                sensor_result.update({SENSORGETDEVICEID.format(BOD, position):
                    ...
                    SENSORGETDATA.format(BOD, position): Conversion().parse_rawdata(
                        sensor_reading.registers,self.bod_data_position[POSITION_1],
                        self.bod_data_position[POSITION_2])})
    ...
    def get_bod_reading(self, bod_start_flag):
        ...
        try:
            ...
            if type(ex).__name__ == CONNECTIONEXCEPTION:
                result = {ERRORMESSAGESENSOR.format(BOD, self.deviceid):MODBUSCONNECTIONERROR}
            else:
                result = {ERRORMESSAGESENSOR.format(BOD, self.deviceid):ex}
        ...
        try:
            if self.bod_configurations is not None:
                ...
                Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(Matching(
                        ...
                        SENSORSERROR.format(BOD, self.deviceid), Log_levels[ERROR], self.deviceid)
            return {BOD:result}
        except IndexError as ex:
            Datalogs.get_instance().logging_error((ex,Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(BOD, self.deviceid),
    ...
    def diasble_reading(self):
        ...
        try:
            ...
            Datalogs.get_instance().logging_error(READING_DISABLED,SENSORSERROR.format(
                BOD, self.deviceid), Log_levels[INFO], self.deviceid)
            ...
            while count < 3:
                with open(SENSOR_ADDRESS_FILE, READ_MODE, encoding="utf-8") as file:
                    ...
                    Datalogs.get_instance().logging_error(ADDED_DEVICE_SUCCESSFULLY,
                                    SENSORSERROR.format(BOD), Log_levels[INFO], self.deviceid)
                    result = ADDED_DEVICE_SUCCESSFULLY
            if count >= 3:
