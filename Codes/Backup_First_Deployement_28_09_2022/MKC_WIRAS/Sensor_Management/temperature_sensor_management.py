...
from general_configurations import FLAG_STATUS_0 , TEMPERATURE, INDEXERROR, VALUEERROR, Error_number
from general_configurations import INFO_NOT_FOUND_IN_FILE, INDENT_LEVEL, INCREMENT_1
from general_configurations import ADDED_DEVICE_SUCCESSFULLY, TEMPERATURE_SENSOR_NOT_FOUND
...
from general_configurations import (
    ...
    ERROR_IN_SET_TEMPERATURE_CONFIGURATIONS,
)
from general_configurations import (
    INFO,
...
from general_configurations import (
    ...
    CONNECTION_COUNT_0 ,
    CONNECTION_COUNT_10 ,
    TEMPERATURE_ARRAY_POS_1 ,
    TEMPERATURE_ARRAY_POS_2
)
...
class TemperatureSensor(object):
    """
    This class contain functions which initilize configurations of Temperature and its array
    position for getting the Temperature data from the Temperature reading.It contain
    functions which reads the data from Temperature Sensor.
    """
    ...
    def __init__(self,deviceid = None):
        ...
        self.temperature_start_flag = FLAG_STATUS_0
        self.temperature_configurations = None
        self.temperature_data_position = [TEMPERATURE_ARRAY_POS_1 , TEMPERATURE_ARRAY_POS_2 ]
        self.temperature_configurations = self.get_temperature_configurations()
    def get_temperature_configurations(self):
        """
        Description: Returns the temperature configurations from Sensor_Configuration_File.
        ...
        try:
            ...
            with open(CONFIGURATIONS_FILE, READ_MODE, encoding="utf-8") as file:
                ...
                if file_data and SENSORCONFIGURATIONS.format(TEMPERATURE) in file_data:
                    for position in range(len(file_data[SENSORCONFIGURATIONS.format(TEMPERATURE)])):
                        config_list.append(
                            list(file_data[SENSORCONFIGURATIONS.format(TEMPERATURE)][
                                position].values()))
                    ...
                    Datalogs().logging_error(Matching().error_mapping(INFO_NOT_FOUND_IN_FILE),
                            SENSORSERROR.format(TEMPERATURE,self.deviceid),
        ...
        try:
            ...
            if self.validate_message(sensor_reading_check):
                ...
                sensor_result.update({SENSORGETDEVICEID.format(TEMPERATURE, position):
                    self.temperature_configurations[position][ARRAY_POSITION_0 ],
                    ERRORMESSAGESENSOR.format(TEMPERATURE, position): result})
            else:
                sensor_result.update({SENSORGETDEVICEID.format(TEMPERATURE, position):
                    self.temperature_configurations[position][ARRAY_POSITION_0 ],
                    SENSORGETDATA.format(TEMPERATURE, position): Conversion().parse_rawdata(
                        sensor_reading.registers,self.temperature_data_position[POSITION_1],
                        self.temperature_data_position[POSITION_2])})
    ...
    def get_temperature_reading(self, temperature_start_flag):
        """
        Description: Returns the final output from the sensor readings
        Input Parameters: temperature_start_flag which is either True or False
        Output Type: dictionary
        """
        try:
            temperature_data = {}
            for position, _ in enumerate(self.temperature_configurations):
                while True:
                    if (temperature_start_flag == FLAG_STATUS_1) and (self.deviceid ==
                                self.temperature_configurations[position][ARRAY_POSITION_0 ]):
                        temperature_reading = (DeviceCommunication.gateway_connect(
                            ).read_holding_registers(
                            count = self.temperature_configurations[position][ARRAY_POSITION_2],
                            address = self.temperature_configurations[position][ARRAY_POSITION_1 ],
                            unit = self.temperature_configurations[position][ARRAY_POSITION_0]))
                        temperature_data.update(self.validate_content(temperature_reading,
                                                                      position))
            ...
            Datalogs.get_instance().logging_error((ex, Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(TEMPERATURE, self.deviceid),
                                                  Log_levels[ERROR], self.deviceid)
            return {ERRORMESSAGESENSOR.format(TEMPERATURE, self.deviceid):ex}
        except Exception as ex:
            if type(ex).__name__ == CONNECTIONEXCEPTION:
                result = {ERRORMESSAGESENSOR.format(TEMPERATURE, self.deviceid):
                    MODBUSCONNECTIONERROR}
            else:
                result = {ERRORMESSAGESENSOR.format(TEMPERATURE, self.deviceid):ex}
    ...
    def enable_reading(self):
        ...
        """
        try:
            if self.temperature_configurations is not None:
                self.temperature_start_flag = FLAG_STATUS_1
                result = self.get_temperature_reading(self.temperature_start_flag)
            else:
    ...
    def diasble_reading(self):
        ...
        try:
            self.temperature_start_flag = FLAG_STATUS_0
            self.get_temperature_reading(self.temperature_start_flag)
            Datalogs.get_instance().logging_error(READING_DISABLED,SENSORSERROR.format(
               TEMPERATURE, self.deviceid), Log_levels[INFO], self.deviceid)
    ...
    def add_device(self, new_slave_id):
        ...
        try:
            ...
            while count < 3:
                with open(SENSOR_ADDRESS_FILE, READ_MODE, encoding="utf-8") as file:
                    ...
                    sensor_address = data[SENSORGETADDRESS[INITIAL_INDEX :FINAL_INDEX ]
                                        ][SENSORGETSTARTADDRESS[:FINAL_INDEX ].format(TEMPERATURE)]
                    sensor_id = data[SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                    ][SENSORGETDEFAULTID[:FINAL_INDEX ].format(TEMPERATURE)]
                    ...
                    Datalogs.get_instance().logging_error(ADDED_DEVICE_SUCCESSFULLY,
                                    SENSORSERROR.format(TEMPERATURE), Log_levels[INFO],
                                    self.deviceid)
                    result = ADDED_DEVICE_SUCCESSFULLY
            if count >= 3:
            ...
            print("999",ex.errno)
            Datalogs.get_instance().logging_error((ex,Error_number[FILENOTFOUND]),
                                                  SENSORSERROR.format(TEMPERATURE),
                                                  Log_levels[DEBUG], self.deviceid)
            return ex
    ...
    def set_temperature_configurations(self, data):
        ...
        try:
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                file_data = json.load(file)
                if list((data).keys())[INDEX_POSITION_0 ] == TEMPERATURE:
                    ...
                    count = COUNT_0
                    for position in range(len(file_data[SENSORCONFIGURATIONS.format(TEMPERATURE)])):
                        if file_data[SENSORCONFIGURATIONS.format(TEMPERATURE)][position][
                    ...
                    if count == COUNT_0 :
                        file_data[SENSORCONFIGURATIONS.format(
                            TEMPERATURE)].append(new_configuration)
                    ...
                    Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(
                        INVALID_INFO, ERROR_IN_SET_TEMPERATURE_CONFIGURATIONS)),SENSORSERROR.format(
                            TEMPERATURE, self.deviceid), Log_levels[ERROR],
                        self.deviceid)
            ...
            print("1000",ex.errno)
            Datalogs.get_instance().logging_error((ex,Error_number[FILENOTFOUND]),
                    SENSORSERROR.format(TEMPERATURE, self.deviceid),Log_levels[ERROR],self.deviceid)
            return ex
        except IndexError as ex:
            Datalogs.get_instance().logging_error((ex,Error_number[FILENOTFOUND]),
                                                  SENSORSERROR.format(TEMPERATURE, self.deviceid),
    ...
    def temperature_reinitiliaze(self):
        """
    ...
    def remove_temperature_configurations(self):
        """
        ...
        try:
            ...
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                ...
                temperature_data = file_data_change[SENSORCONFIGURATIONS.format(TEMPERATURE)]
                for index, _ in enumerate(len(temperature_data)):
                    if temperature_data[index][SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                       ] == self.deviceid:
                        check_con = check_con + INCREMENT_1
                        self.temperature_reinitiliaze()
                        del temperature_data[index]
                        result = CONFIGURATIONS_REMOVED_SUCCESSFULLY
                if check_con == COUNT_0 :
                    Datalogs.get_instance().logging_error(TEMPERATURE_SENSOR_NOT_FOUND.format(
                        self.deviceid), SENSORSERROR.format(TEMPERATURE, self.deviceid),
                                                        Log_levels[ERROR], self.deviceid)
                    result = TEMPERATURE_SENSOR_NOT_FOUND.format(self.deviceid)
                file_data[SENSORCONFIGURATIONS.format(TEMPERATURE)] = temperature_data
                file.truncate(POSTION_0)
                file.seek(POSTION_0)
                json.dump(file_data, file, indent = INDENT_LEVEL)
            return result
