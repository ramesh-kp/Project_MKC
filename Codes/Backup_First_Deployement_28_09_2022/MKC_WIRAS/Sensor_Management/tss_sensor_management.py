...
from general_configurations import FLAG_STATUS_0 , TSS, INDEXERROR, VALUEERROR, Error_number
from general_configurations import INFO_NOT_FOUND_IN_FILE, INDENT_LEVEL, INCREMENT_1
from general_configurations import ADDED_DEVICE_SUCCESSFULLY, TSS_SENSOR_NOT_FOUND
...
from general_configurations import (
    ...
    ERROR_IN_SET_TSS_CONFIGURATIONS,
)
from general_configurations import (
    INFO,
...
class TSSSensor(object):
    """
    This class contain functions which initilize configurations of TSS and its array position
    for getting the TSS data from the TSS reading.It contain functions which reads the data from
    TSS Sensor.
    """
    ...
    def __init__(self,deviceid = None):
        ...
        self.tss_start_flag = FLAG_STATUS_0
        self.tss_configurations = None
        self.tss_data_position = [TSS_ARRAY_POS_1 , TSS_ARRAY_POS_2 ]
        self.tss_configurations = self.get_tss_configurations()
    def get_tss_configurations(self):
        """
        Description: Returns the tss configurations from Sensor_Configuration_File.
    ...
    def validate_content(self, sensor_reading, position):
        ...
        try:
            ...
            if self.validate_message(sensor_reading_check):
                ...
                sensor_result.update({SENSORGETDEVICEID.format(TSS, position):
                    self.tss_configurations[position][ARRAY_POSITION_0 ],
                    ERRORMESSAGESENSOR.format(TSS, position): result})
            else:
                sensor_result.update({SENSORGETDEVICEID.format(TSS, position):
                    self.tss_configurations[position][ARRAY_POSITION_0 ],
                    SENSORGETDATA.format(TSS, position): Conversion().parse_rawdata(
                        sensor_reading.registers,self.tss_data_position[POSITION_1],
                        self.tss_data_position[POSITION_2])})
    ...
    def get_tss_reading(self, tss_start_flag):
        """
        Description: Returns the final output from the sensor readings
        Input Parameters: tss_start_flag which is either True or False
        Output Type: dictionary
        """
        try:
            tss_data = {}
            for position, _ in enumerate(self.tss_configurations):
                while True:
                    if (tss_start_flag == FLAG_STATUS_1) and (self.deviceid ==
                                self.tss_configurations[position][ARRAY_POSITION_0 ]):
                        tss_reading = (DeviceCommunication.gateway_connect().read_holding_registers(
                            count=self.tss_configurations[position][ARRAY_POSITION_2],
                            address=self.tss_configurations[position][ARRAY_POSITION_1 ],
                            unit=self.tss_configurations[position][ARRAY_POSITION_0]))
                        tss_data.update(self.validate_content(tss_reading, position))
                        Datalogs.get_instance().logging_error(RECEIVED_READINGS_FROM_SENSOR,
                                    SENSORSERROR.format(TSS, self.deviceid), Log_levels[INFO],
                                    self.deviceid)
            ...
            Datalogs.get_instance().logging_error((ex, Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(TSS, self.deviceid),
                                                  Log_levels[ERROR], self.deviceid)
            return {ERRORMESSAGESENSOR.format(TSS, self.deviceid):ex}
        except Exception as ex:
            if type(ex).__name__ == CONNECTIONEXCEPTION:
                result = {ERRORMESSAGESENSOR.format(TSS, self.deviceid):
                    MODBUSCONNECTIONERROR}
            else:
                result = {ERRORMESSAGESENSOR.format(TSS, self.deviceid):ex}
    ...
    def enable_reading(self):
        ...
        """
        try:
            if self.tss_configurations is not None:
                self.tss_start_flag = FLAG_STATUS_1
                result = self.get_tss_reading(self.tss_start_flag)
            else:
                ...
                Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(Matching(
                        ...
                        SENSORSERROR.format(TSS, self.deviceid), Log_levels[ERROR], self.deviceid)
            return {TSS:result}
        except IndexError as ex:
            Datalogs.get_instance().logging_error((ex,Error_number[INDEXERROR]),
                                                  SENSORSERROR.format(TSS, self.deviceid),
    ...
    def diasble_reading(self):
        ...
        try:
            self.tss_start_flag = FLAG_STATUS_0
            self.get_tss_reading(self.tss_start_flag)
            Datalogs.get_instance().logging_error(READING_DISABLED,SENSORSERROR.format(
                TSS, self.deviceid), Log_levels[INFO], self.deviceid)
            ...
            while count < 3:
                with open(SENSOR_ADDRESS_FILE, READ_MODE, encoding="utf-8") as file:
                    ...
                    sensor_address = data[SENSORGETADDRESS[INITIAL_INDEX :FINAL_INDEX ]
                                        ][SENSORGETSTARTADDRESS[:FINAL_INDEX ].format(TSS)]
                    sensor_id = data[SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                    ][SENSORGETDEFAULTID[:FINAL_INDEX ].format(TSS)]
                    ...
                    Datalogs.get_instance().logging_error(ADDED_DEVICE_SUCCESSFULLY,
                                    SENSORSERROR.format(TSS), Log_levels[INFO], self.deviceid)
                    result = ADDED_DEVICE_SUCCESSFULLY
            if count >= 3:
    ...
    def set_tss_configurations(self, data):
        ...
        try:
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                file_data = json.load(file)
                if list((data).keys())[INDEX_POSITION_0 ] == TSS:
                    ...
                    count = COUNT_0
                    for position in range(len(file_data[SENSORCONFIGURATIONS.format(TSS)])):
                        if file_data[SENSORCONFIGURATIONS.format(TSS)][position][SENSORGETDEVICEID[
                    ...
                    if count == COUNT_0 :
                        ...
                        file_data[SENSORCONFIGURATIONS.format(TSS)] = sorted(
                    ...
                    Datalogs.get_instance().logging_error((ERROR_CODE_FORMATTER.format(
                        INVALID_INFO, ERROR_IN_SET_TSS_CONFIGURATIONS)),SENSORSERROR.format(
                            TSS, self.deviceid), Log_levels[ERROR],
                        self.deviceid)
    ...
    def remove_tss_configurations(self):
        """
        ...
        try:
            ...
            with open(CONFIGURATIONS_FILE, READ_WRITE_MODE, encoding="utf-8") as file:
                ...
                for index, _ in enumerate(len(tss_data)):
                    if tss_data[index][SENSORGETDEVICEID[INITIAL_INDEX :FINAL_INDEX ]
                                       ] == self.deviceid:
                        check_con = check_con + INCREMENT_1
                        self.tss_reinitiliaze()
                        del tss_data[index]
                        result = CONFIGURATIONS_REMOVED_SUCCESSFULLY
                if check_con == COUNT_0 :
                    Datalogs.get_instance().logging_error(TSS_SENSOR_NOT_FOUND.format(
                        self.deviceid), SENSORSERROR.format(TSS, self.deviceid),
                                                        Log_levels[ERROR], self.deviceid)
                    result = TSS_SENSOR_NOT_FOUND.format(self.deviceid)
