...
from Validation_Messages import validation, errorMessage
import struct
import pytz
...
class Receive_Azure_Data:
    ...
    def Configuration_Add(self, sensor, value):
        ...
        try:
            ...
            Data_Logs().Error_log(errorMessage(validation["MKC2"],sensor))
            return errorMessage(validation["MKC2"])
    ...
    def Azure_Read(self):
        ...
        try:
            ...
            if Total_Length == (Variable_Length + Message_Length + 16):
                ...
                Send_Sensor_Datas().Iothub_Client_Telemetry_Sample_Run(receive_data)
                return receive_data
            else:
                print("The message is not being received appropriately.")
        except:
            Data_Logs().Error_log(errorMessage(validation["MKC5"]))
            errorMessage(validation["MKC5"])
    ...
    def Adding_Sensor_Configuration(self, data):
        ...
        try:
            ...
            if "TDS" in azure_data:
                self.Configuration_Add_Sensor_Details(azure_data, "Conductivity")
        except:
            Data_Logs().Error_log(errorMessage(validation["MKC6"]))
            return errorMessage(validation["MKC6"])
