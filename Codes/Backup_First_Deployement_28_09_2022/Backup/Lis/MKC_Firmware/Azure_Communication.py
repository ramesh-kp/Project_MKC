...
class Send_Sensor_Datas:
    ...
    def Iothub_Client_Telemetry_Sample_Run(self, reading):
        ...
        '''
        try:
            iothub_client = self.Iothub_Client_Init()
    ...
    class Read_Sensor_Datas:
        ...
        def Mqtt_Message_Format(self, sensor_data):
            '''
            Description:        input data is appended to the length of the data in hexdecimal eight digit,To this Topic is added infront.
                                Then find the length of this much in hexadecimal eight digit and appended in front.
                                the topic is appended to      
        ...
        def Full_Sensor_Readings(self):
            ...
            Description:        The result that we obtain from the  Local_Time(),Energymeter_Reading(), COD_Reading(),BOD_Reading(), TSS_Reading(),
                                Temperature_Reading(),Ph_Reading(),TDS_Reading() are passed to the Mqtt_Message_Format() and its output stored in final_data,
...
class Receive_Azure_Data:
    ...
    def Setting_Frequency_Time(self, data):
        ...
        print(type(azure_data))
        if "Time" in azure_data:
            New_Frequency_Time = str(float(azure_data["Time"]))
            self.Configuration_Rewrite("Frequency_Time", New_Frequency_Time)
...
#time = Receive_Azure_Data().Setting_Frequency_Time(azure_data)
energy = Receive_Azure_Data().Adding_Sensor_Configuration(azure_data)
print(energy)
