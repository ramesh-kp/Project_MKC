...
class BODSensor(object):
    ...
    def remove_bod_configurations(self, device_id):
        try:
            check_con = 0
            with open(Configurations_File, "r+") as file:
                ...
                for index in range(len(bod_data)):
                    if bod_data[index][SensorGetDeviceID[3:-3]] == device_id:
                        ...
                        break
                if check_con == 0:
                    print(BOD_Sensor_not_found.format(device_id))
...
#   robots[f"robot_{i}"] = Robot()
    def check(self, DeviceID):
        with open(Configurations_File, "r+") as file:
            file_data = json.load(file)
            for i in range(len(file_data["BOD_Configurations"])):
                if file_data["BOD_Configurations"][i]["Device_ID"] == DeviceID:
                    DeviceID = BODSensor()
                    print(DeviceID)
                    return DeviceID
                # file_data["BOD_Configurations"][i]["Device_ID"] == DeviceID
            #     print(file_data["BOD_Configurations"][i])
            # print(file_data["BOD_Configurations"])
