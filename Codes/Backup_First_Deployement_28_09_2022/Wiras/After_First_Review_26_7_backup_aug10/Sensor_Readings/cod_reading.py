...
class CODSensor(object):
    ...
    def cod_reinitiliaze(self, cod_flag_check):
        if cod_flag_check == True:
            self.cod_configurations = None
        else:
            print("ReIntilization unsucessful due to writing failed.")

    def remove_cod_configurations(self, device_id, cod_flag_check):
        try:
            check_con = 0
            with open(Configurations_File, "r+") as file:
                ...
                for index in range(len(cod_data)):
                    if cod_data[index][SensorGetDeviceID[3:-3]] == device_id:
                        check_con = check_con + 1
                        self.cod_reinitiliaze(cod_flag_check)
                        del cod_data[index]
                        break
                if check_con == 0:
                    print("COD DeviceID {} Not found".format(device_id))
