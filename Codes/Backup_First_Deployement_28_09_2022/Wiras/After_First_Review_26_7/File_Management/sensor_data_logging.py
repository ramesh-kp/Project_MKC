...
class DeviceDataLogging(object):
    ...
    def file_created(cls, files_pos, folder_name):
        ...
        try:
            ...
            Datalogs.getInstance().logging_error(Matching().error_mapping(FILE_NOT_FOUND), SensorsError.format(
                Function), Log_levels[Debug])
            return Matching().error_mapping(FILE_NOT_FOUND)
        except IndexError:
            Datalogs.getInstance().logging_error(Matching().error_mapping(LIST_INDEX_ERROR), SensorsError.format(
                Function),Log_levels[Debug])
